...
const STRINGS = {
  en: {
    ...
    gestureNote: 'If~you do want to~try the camera hand gestures, here is what they need: a~front camera you can stand back from, both hands free inside the frame, and enough light on~them. It~works best on~a~laptop or~a~desktop with a~webcam. On~a~phone or~a~tablet, prop the device up at arm’s length so both hands fit in~view. Allow camera access when the browser asks. Nothing is recorded and nothing leaves your~machine.',
  ...
  ru: {
    ...
    gestureNote: 'Если всё-таки хочешь попробовать жесты, вот что для них нужно: фронтальная камера, от~которой можно отойти, обе руки свободны и~в~кадре, на~них достаточно света. Лучше всего это работает на~ноутбуке или компьютере с~веб-камерой. Телефон или планшет поставь на~расстоянии вытянутой руки, чтобы обе руки помещались в~кадр. Разреши доступ к~камере, когда браузер спросит. Ничего не~записывается и~ничего не~покидает твоё~устройство.',
